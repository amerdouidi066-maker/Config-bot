...
async def run_in_cloudshell_direct(update, lab_url, region):
    start = time.time()
    for attempt in range(3):
        try:
            result = await run_stealth_session(update, lab_url, region, start)
            if result[0]:
                return result
            if attempt < 2:
                await asyncio.sleep(5)
