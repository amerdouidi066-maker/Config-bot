...
# -*- coding: utf-8 -*-
"""
SHADOW LEGION v40.0 – STEALTH ARCHITECT
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
- No /login or /done – cookies are uploaded via the dashboard.
- Advanced fingerprint: Canvas, WebGL, Audio, plugins.
- Natural mouse movement (bezier curves).
- Human-like typing with random mistakes.
- Fake extensions (Google Docs, PDF Viewer).
- Cookie management solely through the web dashboard.
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
"""

import os, re, time, json, base64, random, logging, asyncio, aiohttp
...
if not TOKEN: raise ValueError("❌ TOKEN غير موجود")
MONGO_URI = os.environ.get("MONGO_URI")
if not MONGO_URI: raise ValueError("❌ MONGO_URI غير موجود")
...
logging.basicConfig(format="%(asctime)s - %(levelname)s - %(name)s - %(message)s", level=logging.INFO,
                    handlers=[RotatingFileHandler("bot.log", maxBytes=5*1024*1024, backupCount=3), logging.StreamHandler()])
logger = logging.getLogger(__name__)
logger.info("🔥 SHADOW LEGION v40.0 (Stealth Architect) بدأ التشغيل...")

client = MongoClient(MONGO_URI, serverSelectionTimeoutMS=5000)
db = client[MONGO_DB_NAME]
users_collection, history_collection, cookies_collection = db["users"], db["deploy_history"], db["cookies"]
client.admin.command('ping')
logger.info("✅ الاتصال بـ MongoDB ناجح.")

def get_user(user_id: int) -> Optional[Dict]:
    doc = users_collection.find_one({"_id": user_id})
    if doc: return {"user_id": doc["_id"], "username": doc.get("username"), "first_name": doc.get("first_name"), "last_name": doc.get("last_name"), "deploy_count": doc.get("deploy_count", 0), "last_active": doc.get("last_active"), "joined_at": doc.get("joined_at"), "last_link": doc.get("last_link")}
    return None

def create_or_update_user(user_id: int, username=None, first_name=None, last_name=None):
...
def get_history(user_id: int, limit: int = 10) -> List[Dict]:
    ...
    return [{"id": str(d["_id"]), "lab_url": d.get("lab_url"), "service_url": d.get("service_url"), "vless_link": d.get("vless_link"), "region_used": d.get("region_used"), "deployed_at": d.get("deployed_at"), "success": d.get("success", 0), "error_msg": d.get("error_msg"), "duration": d.get("duration_seconds", 0)} for d in docs]
...
async def save_cookies_to_db(user_id: int, cookies: List[Dict]) -> None:
    try:
        ...
        logger.info(f"🍪 تم حفظ {len(cookies)} كوكي للمستخدم {user_id}")
    except Exception as e: logger.error(f"❌ فشل حفظ الكوكيز: {e}")
...
async def load_cookies_from_db(user_id: int) -> List[Dict]:
    try:
        ...
        if doc and doc.get("data"):
            ...
            logger.info(f"🍪 تم تحميل {len(cookies)} كوكي للمستخدم {user_id}")
            return cookies
    except Exception as e: logger.warning(f"⚠️ فشل تحميل الكوكيز: {e}")
    return []
...
async def cleanup_session(user_id: int) -> None:
    ...
    logger.info(f"🧹 تنظيف جلسة المستخدم {user_id}")
    session.stop_event.set()
    if session.stream_task and not session.stream_task.done():
        session.stream_task.cancel()
        try: await asyncio.wait_for(session.stream_task, timeout=2.0)
        except: pass
    for obj in (session.page, session.context, session.browser):
        if obj:
            try: await obj.close()
            except: pass
    logger.info(f"✅ تم تنظيف جلسة المستخدم {user_id}")
...
def get_random_proxy() -> Optional[str]: return random.choice(PROXY_LIST) if PROXY_LIST else None
def get_random_referer() -> str: return random.choice(["https://www.google.com/", "https://accounts.google.com/", "https://mail.google.com/", "https://www.cloudskillsboost.google.com/"])

def generate_random_fingerprint() -> Dict:
    chrome_versions = ["126.0.6478.126", "125.0.6422.142", "124.0.6367.218", "123.0.6312.122", "122.0.6261.129"]
    return {
        "vendor": random.choice(['Intel Inc.', 'NVIDIA Corporation', 'AMD', 'Apple']),
        "renderer": random.choice(['Intel Iris OpenGL Engine', 'NVIDIA GeForce GTX 1660', 'AMD Radeon Pro 5500M', 'Intel UHD Graphics 620']),
        "canvas_noise": random.uniform(0.01, 0.08),
        "audio_noise": random.uniform(0.0005, 0.003),
        ...
        "webgl_renderer": random.choice([
            "ANGLE (Intel, Intel(R) UHD Graphics 620 (0x0000591B) Direct3D11)",
...
async def create_stealth_context(user_id: int, browser) -> Tuple[Any, Any]:
    ...
    width = random.randint(1800, 1920); height = random.randint(1000, 1080)
    tz = random.choice(TIMEZONES); lang = random.choice(LANGUAGES)
    lat = random.uniform(30, 50); lon = random.uniform(-100, -70)

    context_options = {
        "user_agent": ua, "viewport": {"width": width, "height": height},
        "locale": lang.split(",")[0], "timezone_id": tz,
        "permissions": ["geolocation"], "geolocation": {"latitude": lat, "longitude": lon},
        "extra_http_headers": {
            "Accept-Language": lang, "Accept-Encoding": "gzip, deflate, br",
            "Sec-Ch-Ua": f'"Google Chrome";v="{fp["chrome_version"].split(".")[0]}", "Chromium";v="{fp["chrome_version"].split(".")[0]}", "Not?A_Brand";v="99"',
            "Sec-Ch-Ua-Mobile": "?0",
            "Sec-Ch-Ua-Platform": f'"{fp["platform"].replace("Win32","Windows").replace("MacIntel","macOS").replace("Linux x86_64","Linux")}"',
            "Upgrade-Insecure-Requests": "1", "User-Agent": ua, "Referer": get_random_referer(), "Origin": "https://console.cloud.google.com"
        },
        "ignore_https_errors": True, "accept_downloads": True, "java_script_enabled": True,
        "bypass_csp": True, "record_video_dir": "videos" if RECORD_VIDEO else None,
    }
    proxy = get_random_proxy()
    if proxy: context_options["proxy"] = {"server": proxy}
    context = await browser.new_context(**context_options)

    cookies = await load_cookies_from_db(user_id)
    if cookies: await context.add_cookies(cookies)
    else: logger.warning(f"⚠️ لا توجد كوكيز للمستخدم {user_id}")
    ...
    await context.add_init_script(f"""
        (function() {{
            ...
            if (!window.chrome) {{
                window.chrome = {{ runtime: {{}}, loadTimes: () => {{}}, csi: () => {{}}, app: {{}}, webstore: {{ onInstallStageChanged: () => {{}}, onDownloadProgress: () => {{}} }} }};
            }}
            const pluginData = [
                {{ name: 'Chrome PDF Plugin', filename: 'internal-pdf-viewer', description: 'Portable Document Format' }},
                {{ name: 'Chrome PDF Viewer', filename: 'mhjfbmdgcfjbbpaeojofohoefgiehjai', description: '' }},
                {{ name: 'Native Client', filename: 'internal-nacl-plugin', description: '' }},
                {{ name: 'Google Docs Offline', filename: 'ghbmnnjooekpmoecnnnilnnbdlolhkhi', description: '' }},
                {{ name: 'Google Drive', filename: 'apdfllckaahabafndbhieahigkjlhalf', description: '' }},
                {{ name: 'Google Hangouts', filename: 'nckgahadagoaajjgafhacjanaoiihapd', description: '' }}
            ];
            const plugins = pluginData.map(p => {{
                const mimeTypes = [];
                if (p.name.includes('PDF')) mimeTypes.push({{ type: 'application/pdf', suffixes: 'pdf', description: 'PDF' }});
                return {{ name: p.name, filename: p.filename, description: p.description, length: mimeTypes.length, item: (i) => mimeTypes[i] || null, namedItem: (n) => mimeTypes.find(m => m.type === n) || null }};
            }});
            plugins.length = pluginData.length;
            navigator.__defineGetter__('plugins', () => plugins);
            Object.defineProperty(navigator, 'languages', {{ get: () => ['{lang.split(",")[0]}', 'en-US', 'en'] }});
            ...
            HTMLCanvasElement.prototype.toDataURL = function(type, quality) {{
                ...
                for (let i = 0; i < data.length; i += 4) {{
                    data[i] = Math.min(255, data[i] + (Math.random() - 0.5) * noise * 255);
                    data[i+1] = Math.min(255, data[i+1] + (Math.random() - 0.5) * noise * 255);
                    data[i+2] = Math.min(255, data[i+2] + (Math.random() - 0.5) * noise * 255);
                }}
            ...
            delete window.__pw_;
            console.log('[ARCHITECT] بصمة متطورة محملة.');
        }})();
    """)
    page = await context.new_page()
    await simulate_natural_mouse(page)
    return context, page

async def simulate_natural_mouse(page):
    try:
        width, height = 1920, 1080
        sx, sy = random.randint(100,500), random.randint(100,500)
        for _ in range(random.randint(5,10)):
            ex, ey = random.randint(100,width-100), random.randint(100,height-100)
            steps = random.randint(20,40)
            for t in range(steps):
                progress = t / steps
                eased = progress * progress * (3 - 2 * progress)
                x = sx + (ex - sx) * eased + random.uniform(-5, 5)
                y = sy + (ey - sy) * eased + random.uniform(-5, 5)
    ...
    while time.time() - start < max_wait:
        try:
            ...
            if "console.cloud.google.com" in url or "shell.cloud.google.com" in url:
                if "sign in" not in text.lower(): return True, ""
...
async def click_start_ultimate(page, max_attempts=6) -> bool:
    ...
    for attempt in range(max_attempts):
        try: await page.evaluate("window.scrollTo(0, document.body.scrollHeight * 0.5)")
        except: pass
    ...
    for attempt in range(3):
        try:
            ...
            for sel in [".xterm-helper-textarea", ".xterm", ".terminal", "[role='textbox']"]:
                try: await page.focus(sel); break
                except: continue
            await asyncio.sleep(random.uniform(0.3, 0.8))
            for ch in cmd:
                await page.keyboard.type(ch, delay=random.randint(20, 80))
        ...
        while not session.stop_event.is_set():
            try:
                ...
                if screenshot:
                    ...
                    try: stream_state.update_status(project=page.url[:80])
                    except: pass
                await asyncio.sleep(0.2)
            except Exception as e:
                if "closed" in str(e).lower() or "Target page" in str(e):
                    logger.warning(f"⚠️ المتصفح مغلق، إيقاف البث")
                    break
                logger.warning(f"⚠️ خطأ في البث: {e}")
                await asyncio.sleep(0.5)
    except asyncio.CancelledError:
        logger.info(f"⏹️ تم إلغاء البث للمستخدم {session.user_id}")
    finally:
...
with open("index.html", "w") as f:
    f.write("<h1>Shadow Legion v40</h1><p>Stealth Architect deploy.</p>")
        ...
        async with async_playwright() as p:
            session.browser = await p.chromium.launch(
                ...
                args=["--no-sandbox", "--disable-dev-shm-usage", "--disable-blink-features=AutomationControlled",
                      "--disable-gpu", "--disable-software-rasterizer", "--disable-features=IsolateOrigins,site-per-process",
                      ...
                      "--disable-extensions", "--disable-features=TranslateUI", "--disable-ipc-flooding-protection",
            ...
            if not ok: return False, "", msg, int(time.time()-start_time), ""
            ...
            for _ in range(5):
                if await click_start_ultimate(page, max_attempts=1):
                    clicked = True; break
                await asyncio.sleep(4)
            if not clicked: return False, "", "⚠️ لم يتم العثور على زر Start", int(time.time()-start_time), ""
            ...
            try:
                ...
                if term: result_content = await term.inner_text()
            except: pass
            if not result_content: result_content = await page.inner_text("body")

            if RECORD_VIDEO and session.context:
                try:
                    video = await session.context.video
                    if video: video_path = await video.path()
                except: pass
...
async def deploy_and_report(user_id: int, lab_url: str, region: str, bot, chat_id: int) -> None:
    try:
        success, service, vless, duration, video = await run_stealth_session(user_id, lab_url, region, bot, chat_id)
        ...
        if success:
            ...
            await bot.send_message(chat_id, f"✅ **تم التنفيذ بنجاح**\n🌍 {region_name}\n⏱️ {duration} ثانية\n🌐 `{service}`\n\n🔗 **VLESS:**\n`{vless}`", parse_mode="Markdown")
            ...
            await bot.send_message(chat_id, f"❌ **فشل التنفيذ**\n\n```\n{vless}\n```", parse_mode="Markdown")
    except Exception as e:
        logger.error(f"❌ خطأ في deploy_and_report: {e}")
...
def cleanup_old_recordings():
    try:
        ...
        for filename in os.listdir(recordings_dir):
            ...
            if os.path.isfile(filepath) and datetime.fromtimestamp(os.path.getmtime(filepath)) < cutoff:
                os.remove(filepath)
                logger.info(f"🗑️ تم حذف فيديو قديم: {filename}")
    except Exception as e: logger.warning(f"⚠️ فشل تنظيف الفيديوهات: {e}")

WAITING_LINK, WAITING_REGION = range(2)
KNOWN_REGIONS = {
    "us-central1": "🇺🇸 أيوا (Iowa)", "us-east1": "🇺🇸 ساوث كارولينا (S. Carolina)",
    "us-east4": "🇺🇸 شمال فيرجينيا (N. Virginia)", "us-west1": "🇺🇸 أوريغون (Oregon)",
    "europe-west1": "🇧🇪 بلجيكا (Belgium)", "europe-west2": "🇬🇧 لندن (London)",
    "europe-west3": "🇩🇪 فرانكفورت (Frankfurt)", "europe-west4": "🇳🇱 هولندا (Netherlands)",
    "europe-west6": "🇨🇭 سويسرا (Switzerland)", "europe-west8": "🇮🇹 إيطاليا (Italy)",
    "europe-north1": "🇸🇪 السويد (Sweden)", "asia-east1": "🇹🇼 تايوان (Taiwan)",
    "asia-northeast1": "🇯🇵 طوكيو (Tokyo)", "asia-southeast1": "🇸🇬 سنغافورة (Singapore)",
    "australia-southeast1": "🇦🇺 سيدني (Sydney)", "southamerica-east1": "🇧🇷 ساو باولو (Sao Paulo)",
}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("⚡ **Shadow Legion v40**\n━━━━━━━━━━━━━━━━\nأرسل الرابط، وسأتكفل بالباقي.\n📌 يتم تحميل الكوكيز من قاعدة البيانات (يمكنك رفعها عبر لوحة التحكم).", parse_mode="Markdown")
...
async def receive_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text in ["❌ إلغاء", "🔄 إعادة المحاولة"]:
        if text == "🔄 إعادة المحاولة": return await retry_command(update, context)
...
async def region_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    region = random.choice(list(KNOWN_REGIONS.keys())) if raw == "random" else raw
    lab = context.user_data.get("lab_url")
    if not lab:
        await query.edit_message_text("❌ لا يوجد رابط. أعد الإرسال.")
        return
    region_name = KNOWN_REGIONS.get(region, region)
    await query.edit_message_text(f"⏳ **جاري تجهيز النشر على {region_name}...**\nسأبلغك فور الانتهاء (قد يستغرق 3-6 دقائق).", parse_mode="Markdown")
    asyncio.create_task(deploy_and_report(query.from_user.id, lab, region, context.bot, query.message.chat_id))
...
async def history_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, h in enumerate(history, 1):
        text += f"{i}. {'✅' if h['success'] else '❌'} {KNOWN_REGIONS.get(h['region_used'], h['region_used'])} – {h['deployed_at'][:16]}\n"
    await update.message.reply_text(text, parse_mode="Markdown")

async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("❓ **الأوامر:**\n/start – القائمة الرئيسية\n/deploy – إرسال رابط للنشر\n/retry – إعادة استخدام آخر رابط\n/stats – إحصائياتك\n/history – سجل النشرات\n/cancel – إلغاء\n\n📌 **ملاحظة:** تم حذف /login و /done نهائياً. يتم إدارة الكوكيز عبر لوحة التحكم.", parse_mode="Markdown")

async def fallback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = update.message.text
    if text == "🚀 نشر جديدة": return await deploy_command(update, context)
    elif text == "📊 إحصائياتي": return await stats_command(update, context)
    elif text == "📜 سجل النشر": return await history_command(update, context)
    elif text == "❓ مساعدة": return await help_command(update, context)
    elif text == "🔄 إعادة المحاولة": return await retry_command(update, context)
    elif text == "❌ إلغاء": return await cancel(update, context)
    else: return await receive_link(update, context)

def start_web_dashboard():
    try:
        import threading, web_dashboard
    ...
    except Exception as e: logger.error(f"❌ فشل تشغيل لوحة التحكم: {e}")
...
def main():
    ...
    logger.info("🔥 SHADOW LEGION v40.0 (Stealth Architect) جاهز للتشغيل")
    app.run_polling()
